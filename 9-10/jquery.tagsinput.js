...
$('.inpCom').change(function(event) {
	var num = parseInt($(this).val());
	var singel = $(this).parent().siblings('td').find('.singel').text();
	var pro_total = $(this).parent().siblings('td').find('.pro_total');
	var singel_price = parseInt(singel);
	var pro_price = num*singel_price;
	$(this).siblings('.inpCom').val(num);
	pro_total.text(pro_price);
	totalprice();
});
$('.plus').click(function (){
	var num = parseInt($(this).siblings('.inpCom').val());
	num+=1;
	var singel = $(this).parent().siblings('td').find('.singel').text();
	var pro_total = $(this).parent().siblings('td').find('.pro_total');
	var singel_price = parseInt(singel);
...
function totalprice(){
	...
	var packagePrice=parseInt($('#packagePrice').text());
	for(i=0;i<$('.inpCom').length;i++){
		totalNum += parseInt($('.inpCom').eq(i).val());
		totalPrice += parseInt($('.pro_total').eq(i).text());
	}
	$('#totalNum').text(totalNum);
	$('#totalPrice').text('¥'+totalPrice);
	$('.priceDisplay').text('¥'+(totalPrice+packagePrice));
}
